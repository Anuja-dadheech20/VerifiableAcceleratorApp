  ...
  async retrieveRecord() {
    try {
      ...
      if (file != null) {
        ...
        this.fileData.ContentDocumentLinkRecordId =
          file.contentDocumentLinkRecordId;
      ...
      this.dispatchEvent(
        new CustomEvent("recordretreiveevent", {
          detail: { retreived: true, data: this.recordId, stepname: "cv" }
        })
      );
    } catch (error) {
      console.log(error);
      this.dispatchEvent(
        new CustomEvent("recordretreiveevent", {
          detail: { retreived: false, data: undefined, stepname: "cv" }
        })
      );
      this.loaded = true;
    }
    ...
    if (
      ...
      reader.onload = () => {
        ...
        if (this.fileData != undefined) {
          this.recordToDelete =
            this.fileData.ContentDocumentId != undefined
              ? this.fileData.ContentDocumentId
              : undefined;
        }
    ...
    try {
      ...
      if (this.fileData != undefined) {
        ...
        await uploadFile({
          ...
          cvId: this.fileData.Id,
          cdlId: this.fileData.ContentDocumentLinkRecordId
        });
      }
      this.loaded = true;
      this.dispatchEvent(
        new CustomEvent("stepcompletionevent", {
          detail: { isCompleted: true, data: this.recordId, stepname: "cv" }
        })
      );
    } catch (error) {
      console.log(error);
      this.dispatchEvent(
        new CustomEvent("stepcompletionevent", {
          detail: { isCompleted: false, data: undefined, stepname: "cv" }
        })
      );
      this.loaded = true;
    }
